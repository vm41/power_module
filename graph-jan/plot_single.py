...
NUMBER_OF_SPEEDS=1
SPEED_SETUPS = [5.0]*30
SPEED_MARKER_THRESHHOLD=1.0     #first time and last time having targetSpeed - threshold, will be marked
...
trials=0
for f in fileList:#total log files
    if (f.endswith('.log') and f.startswith('log')):
        ...
        fv=open (LOG_DIR+'/'+myFile,"r") #opening log file
        ...
        counted=0
        TIME_REFERENCE=0
        while True:#reading line by line of the log file
            try:
                ...
                if l=='':
                    break
                l=l.strip().split()
                now=float(l[0])  #storing the refernece time stamp

                if (TIME_REFERENCE==0):#set initial time frame
                    TIME_REFERENCE=now
                ...
                event_type=int(l[1]) #second column in the log file

                if event_type==EVENT_TYPE.MEASUREMENT: #value 1
                    channel=int(l[2])
                    curr=float(l[3])
                    reading_time[trials][channel].append(time)
                    reading[trials][channel].append(curr)
                    lastValue[channel]=curr #0,1,2,3 indices
                    micro_time[trials].append(time)#time showing the total current
        ...
        trials += 1	#means each log file read
...
for this_trial in range(trials):
    ...
    if (SAVE_GRAPH_CHANNELS):
        ...
        lgd = ax1.legend(loc=9, bbox_to_anchor=(0.5, -0.1), ncol=4)
        ax1.set_xlabel('Time (s)')
        ax1.set_ylabel('Current (A)')
        plt.title('%d Channels - Round = %d Speed = %.1f m/s'%(NUMBER_OF_CHANNELS,this_round+1,SPEED_SETUPS[this_speed]))
        plt.savefig('%s/%s/all/%s_channels.png'%(LOG_DIR,STR_GRAPHS,file_names[this_trial]),dpi=600,format="png",bbox_extra_artists=(lgd,), bbox_inches='tight')
        if (SAVE_GRAPH_ROUND_BASED):
    ...
    if (SAVE_GRAPH_TOTAL):
        ...
        # ax2.set_xlim(0,28)
        ax2.set_ylim(min(total_curr[this_trial])-0.5,max(total_curr[this_trial])+0.5)
        ax2.set_xlim(0,micro_time[this_trial][-1]+0.5)
        yticks = ax2.yaxis.get_major_ticks()
#        yticks[-1].label1.set_visible(False)
        title = 'Total Current for Round #'+str(this_trial+1)
        plt.title(title)# - Round = %d Speed = %.1f m/s'%(this_round+1, SPEED_SETUPS[this_speed]))
        plt.tight_layout()

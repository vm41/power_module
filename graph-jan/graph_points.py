...
STR_GRAPH_TITLE = "Total Energy Comparison Over %d Trials per Setup"%(N_DAYS*N_ROUNDS)
...
if TARGET_SETUP == "Angle":
    ...
    SETUP_X_LABEL = "Angle (deg)"    # "Speed (m/s)"  or "Angle (deg)"
...
BAR_LINE_WIDTH=3
BAR_WIDTH = 0.10       # the width of the bars
...
for j in range(N_DAYS):
    for i in range(N_SETUPS):
            for k in range(N_ROUNDS):
                points_x.append(i)
                points_y.append(data[COLUMN_TO_SHOW][j*(N_ROUNDS*N_SETUPS) + i*N_ROUNDS+k])

plt.plot(points_x, points_y, 'ro')


#rects=ax1.bar(ind_setups + 7*BAR_WIDTH, means[COLUMN_TO_SHOW][:SETUPS_TO_SHOW], 2*BAR_WIDTH, color="0.75", hatch='', log=False, linewidth=BAR_LINE_WIDTH,
...
ax1.set_xticks(ind_setups+7*BAR_WIDTH)
...
#plt.tight_layout()
plt.savefig(LOG_DIR+"/"+STR_GRAPHS+"/points_"+TARGET_SETUP+"_"+STR_FILENAME,dpi=600) 

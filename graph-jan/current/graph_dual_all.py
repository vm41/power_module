...
N_SETUPS = 5 #number of setup/distance/speed/turn
SETUPS_TO_SHOW = 5  # for example N_SETUP may be 4, but you don't want the last one.
...
LEGENDS=["Power", "Time"]
COLUMN_TO_SHOW = [9, 5] #each time one graph, this is the column number change this to generate new ones. start from 0, it is adjusted by FIRST_DATA_COLUMN
...
STR_GRAPH_TITLE = "Power and Time Comparison Over %d Trials"%(N_DAYS*N_ROUNDS)
...
STR_Y_LABELS = ["Average Power (W)", "Total Time (s)"]

STR_FILENAME = "power_time.eps"
#"power_time.png"
...
rects1=ax1.bar(ind_setups-2*BAR_WIDTH, means[COLUMN_TO_SHOW[0]][:SETUPS_TO_SHOW], 2*BAR_WIDTH, color="1", hatch='', log=False, linewidth=BAR_LINE_WIDTH,
...
ax2=ax1.twinx()
rects2=ax2.bar(ind_setups, means[COLUMN_TO_SHOW[1]][:SETUPS_TO_SHOW], 2*BAR_WIDTH, color="1", hatch='///', log=False, linewidth=BAR_LINE_WIDTH,

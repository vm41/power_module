...
for this_trial in range(trials):
    ...
    if (TARGET_SETUP=="Speed"):
        ...
        # added_setup_label = 'Voltage log: 3 Cells Bat 1 drainage on 1500 Thrust'
        # added_setup_label = 'Voltage log: 3 Cells Bat 2 drainage on 1500 Thrust'
        added_setup_label = 'Voltage log: 4 Cells Bat drainage on 1500 Thrust'
    else:
    ...
    if (SAVE_GRAPH_CHANNELS):
        ...
        for i in range(NUMBER_OF_CHANNELS):
	    if(i==2 or i==3 or i==0 or i==1 or i==6 or i==7 or i==4):
		#print(i)
		continue
	    ...
	    #ax1.set_xlim(0,120)
            ax1.set_ylim(4,18)
            pass
        plt.yticks(np.arange(4, 17, 0.5))
        ax1.set_xlabel('Time (s)')
        ax1.set_ylabel('Voltage (Volts)')
	lgd = ax1.legend(loc='upper right')
        plt.grid()
	...
	plt.title('Round %d: %s'%(this_trial+1, added_setup_label))# - Round = %d %s'%(NUMBER_OF_CHANNELS,this_day*NUMBER_OF_ROUNDS+this_round+1,added_setup_label))
        ...
        plt.savefig('%s/%s/all/%s_channels.png'%(LOG_DIR,STR_GRAPHS,file_names[this_trial]),bbox_extra_artists=(lgd,),dpi=600,format="png")
        if (SAVE_GRAPH_ROUND_BASED):
